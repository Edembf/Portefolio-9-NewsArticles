        ...
        <ScrollView contentContainerStyle={styles.scrollContainer}>
          {articles.length === 0 ? (
            ...
            articles.map((article, index) => (
              <View key={`${article.title}-${index}`} style={styles.articleCard} testID="articleCard">
                <TouchableOpacity onPress={() => handleArticleClick(article)} testID="handleArticleBtn">
                   {article.image && (
                    <Image
...
const styles = StyleSheet.create({
  ...
  addButton: {
    ...
    borderRadius: 5,
    width: 300,
    marginLeft: wp(45) - 150, // Center the button horizontally
    marginBottom: hp(2),
  },
